...
import { getColumns } from '@/app/shared/invoice/invoice-list/showtime-columns';
...
export default function ShowtimeTable({ data = [] }: { data: any[] }) {
  ...
  const doneShowtimeHandler = async (id) => {
    const { data } = await axios.patch(API_SHOWTIME + '/showtimes/' + id);
    toast.success(<Text as="b">Hoàn thành lịch chiếu thành công</Text>);
  };
  ...
  const onDoneShowtime = useCallback((id: string) => {
    doneShowtimeHandler(id);
  }, []);
