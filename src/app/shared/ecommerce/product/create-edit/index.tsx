...
export default function CreateEditProduct({
  ...
  const onSubmit: SubmitHandler<CreateProductInput> = (data) => {
    ...
    setTimeout(async () => {
      ...
      // console.log('data', data?.description?.replace(/<\/?p>/g, ''));
      // console.log('dataFormat', new Date(data.availableDate).toISOString());

      const res = await axios.post(API_MOVIE + '/movie/create', {
        name: data.title,
        trailer: data.sku,
        image: data?.productImages?.[0].url,
        description: data.description.replace(/<\/?p>/g, ''),
        group: 'GP01',
        openingDay: new Date(data.availableDate).toISOString(),
        rating: 9,
        hot: data.freeShipping,
        nowShowing: data.productAvailability === 'showing' ? true : false,
        comingSoon: data.productAvailability === 'soon' ? true : false,
      });

      console.log('data', res);

      toast.success(
        <Text as="b">Thêm phim thành công {slug ? 'updated' : 'created'}</Text>
      );
      methods.reset();
    }, 600);
  };
    ...
    <div className="@container">
      ...
      <FormProvider {...methods}>
        ...
        >
          ...
          <FormFooter
            isLoading={isLoading}
            submitBtnText={slug ? 'Cập Nhập Phim' : 'Thêm Phim'}
          />
        </form>
      </FormProvider>
    </div>
