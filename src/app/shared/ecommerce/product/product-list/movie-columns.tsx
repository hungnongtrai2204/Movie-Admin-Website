...
// get status badge
function getStatusMovie(status: string) {
  switch (status.toLowerCase()) {
    case 'soon':
      return (
        <div className="flex items-center">
          <Badge color="warning" renderAsDot />
          <Text className="ms-2 font-medium text-orange-dark">Sắp Chiếu</Text>
        </div>
      );
    case 'showing':
      return (
        <div className="flex items-center">
          <Badge color="success" renderAsDot />
          <Text className="ms-2 font-medium text-green-dark">Đang Chiếu</Text>
        </div>
      );
    default:
      return (
        <div className="flex items-center">
          <Badge renderAsDot className="bg-gray-400" />
          <Text className="ms-2 font-medium text-gray-600">Hết Chiếu</Text>
        </div>
      );
  }
}
...
export const getColumns = ({
  ...
  // },
  {
    title: (
      <HeaderCell
        ...
        sortable
        ascending={
          sortConfig?.direction === 'asc' && sortConfig?.key === 'openingDay'
        }
      />
    ),
    onHeaderCell: () => onHeaderCellClick('openingDay'),
  ...
  {
    title: <HeaderCell title="Đánh Giá" />,
    dataIndex: 'rating',
    key: 'rating',
    width: 200,
  ...
  },
  {
    ...
    key: 'status',
    width: 120,
    render: (value: string) => getStatusMovie(value),
  },
  {
    ...
    render: (_: string, row: ProductType) => (
      <div className="flex items-center justify-end gap-3 pe-4">
        <Tooltip
          size="sm"
          content={'Chỉnh Sửa Phim'}
          placement="top"
          color="invert"
        >
          <Link href={routes.movie.editMovie(row.id)}>
